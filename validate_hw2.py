...
def score_hw2( base_url):
    ...
    response = requests.post(base_url+'/historical/', data='{"DATE":"20170601","TMIN":33,"TMAX":44}',headers = {'Content-type': 'application/json'} )
    total_score=total_score+  validate_post(response,'20170601')
    # Checking if the TMAX has been updated for 20170601
    response = requests.get(base_url+'/historical/20170601')
    total_score=total_score+  validate_get_tmax(response, 44)
    # Checking DELETE 
    response=requests.delete(base_url+'/historical/20170601')
    ...
    response=requests.get(base_url+'/forecast/20170209')
    total_score=100*total_score/130.0 +  validate_forcasts(response,'20170209')
